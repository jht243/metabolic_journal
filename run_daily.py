...
Phases:
  1. IndexNow distribution
  2. SEO audit + auto-fix
  3. Sitemap audit & sync (add missing routes, remove dead links, push fixes)
"""
...
@click.option("--skip-sitemap", is_flag=True, help="Skip sitemap sync phase")
@click.option("--max-pages", default=200, type=int, help="Maximum pages for SEO audit crawl")
def main(dry_run: bool, skip_indexnow: bool, skip_autofix: bool, skip_sitemap: bool, max_pages: int) -> None:
    ...
    # ── Phase 3: Sitemap audit & sync ────────────────────────────────
    console.print("\n[bold cyan]Phase 3:[/bold cyan] Sitemap audit & sync...")
    if skip_sitemap:
        results["sitemap_sync"] = {"status": "skipped", "reason": "--skip-sitemap"}
        console.print("  [dim]Skipped[/dim]")
    else:
        try:
            from scripts.sync_sitemap import run_sync

            sitemap_result = run_sync(dry_run=dry_run, spot_check_enabled=True)
            results["sitemap_sync"] = sitemap_result
            if "error" in sitemap_result:
                console.print(f"  [yellow]![/yellow] Sitemap sync error: {sitemap_result['error']}")
            else:
                added = sitemap_result.get("added", 0)
                removed = sitemap_result.get("removed", 0)
                dead = sitemap_result.get("dead_links", 0)
                console.print(
                    f"  [green]OK[/green] Sitemap sync: +{added} added, -{removed} removed, "
                    f"{dead} dead link(s)"
                )
                if sitemap_result.get("pushed"):
                    console.print("  [green]✓[/green] Changes committed and pushed")
        except Exception as exc:
            logger.warning("Sitemap sync failed: %s", exc, exc_info=True)
            results["sitemap_sync"] = {"status": "error", "error": str(exc)}
            console.print(f"  [yellow]![/yellow] Sitemap sync failed (non-fatal): {exc}")
